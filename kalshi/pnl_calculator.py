...
class PNLCalculator:
    ...
    def calculate_position_pnl(self) -> pd.DataFrame:
        ...
        position_pnl.columns = [
            "_".join(col).strip() for col in position_pnl.columns.values
        ]

        # Calculate additional metrics
        position_pnl["total_trades"] = position_pnl["gross_pnl_count"]
        position_pnl["win_rate"] = (
            position_pnl["is_win_sum"] / position_pnl["total_trades"] * 100
        ).round(2)
        position_pnl["avg_win"] = (
            trades[trades["is_win"]].groupby(groupby_col)["net_pnl"].mean().fillna(0)
        )
        position_pnl["avg_loss"] = (
            trades[trades["is_loss"]].groupby(groupby_col)["net_pnl"].mean().fillna(0)
        )
    ...
    def calculate_daily_pnl(self) -> pd.DataFrame:
        ...
        daily_pnl["win_rate"] = (
            daily_pnl["is_win"] / daily_pnl["total_trades"] * 100
        ).round(2)
    ...
    def calculate_summary_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "profit_factor": (
                round(profit_factor, 2) if profit_factor != float("inf") else "Infinite"
            ),
            ...
            "avg_daily_pnl": (
                round(total_pnl / max(trading_days, 1), 2) if trading_days > 0 else 0
            ),
        }
