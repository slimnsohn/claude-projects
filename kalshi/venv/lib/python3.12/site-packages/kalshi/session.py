...
class Session:
    ...
    def __init__(
        self, email=None, password=None, endpoint="https://trading-api.kalshi.com/v1"
    ):
        if email is None:
        ...
        res = requests.post(
            endpoint + "/log_in", json={"email": email, "password": password}
        )
        if res.status_code != 200:
            raise RuntimeError(
                "kalshi.Session failed to log in (%s) (%s)"
    ...
    def _http_op(self, op, path, obj=None):
        ...
        if res.status_code != 200:
            raise RuntimeError(
                "Error from Kalshi API (%s) (%s)" % (res.status_code, res.text)
            )
        return res.json()
    ...
    def get_market_history_cached(self, market_id, last_seen_ts=None):
        ...
        """
        return self._http_op(
            "get",
            f"/cached/markets/{market_id}/stats_history",
            dict((x, y) for x, y in [("last_seen_ts", last_seen_ts)] if y is not None),
        )
    ...
    def get_market_history(self, market_id, last_seen_ts=None):
        ...
        return self._http_op(
            "get",
            f"/markets/{market_id}/stats_history",
            dict((x, y) for x, y in [("last_seen_ts", last_seen_ts)] if y is not None),
        )
    ...
    def user_get_account_history(
        self,
        ShouldReturnDeposits=None,
        ShouldReturnWithdrawals=None,
        ShouldReturnOrders=None,
        ShouldReturnSettlements=None,
        ShouldReturnTrades=None,
        Limit=None,
        user_id=None,
    ):
        ...
        return self._http_op(
            "get",
            f"/users/{user_id}/account/history",
            dict(
                (x, y)
                for x, y in [
                    ("ShouldReturnDeposits", ShouldReturnDeposits),
                    ("ShouldReturnWithdrawals", ShouldReturnWithdrawals),
                    ("ShouldReturnOrders", ShouldReturnOrders),
                    ("ShouldReturnSettlements", ShouldReturnSettlements),
                    ("ShouldReturnTrades", ShouldReturnTrades),
                    ("Limit", Limit),
                ]
                if y is not None
            ),
    ...
    def get_user_deposits(self, page_size=None, page_number=None, user_id=None):
        ...
        return self._http_op(
            "get",
            f"/users/{user_id}/deposits",
            dict(
                (x, y)
                for x, y in [("page_size", page_size), ("page_number", page_number)]
                if y is not None
            ),
        )
    ...
    def notification_mark_read(self, notification_id, user_id=None):
        ...
        user_id = user_id or self.user_id
        return self._http_op(
            "put", f"/users/{user_id}/notifications/{notification_id}/read", None
        )
    ...
    def user_orders_get(
        ...
        min_price=None,
        max_price=None,
        min_place_count=None,
        max_place_count=None,
        min_initial_count=None,
        max_initial_count=None,
        min_remaining_count=None,
    ...
    def user_order_decrease(self, order_id, user_id=None):
        ...
        user_id = user_id or self.user_id
        return self._http_op(
            "post", f"/users/{user_id}/orders/{order_id}/decrease", None
        )
