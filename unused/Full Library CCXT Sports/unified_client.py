...
class UnifiedMarketDataClient:
    ...
    async def get_unified_games(self, sport: Sport,
                               date_from: Optional[datetime] = None,
                               date_to: Optional[datetime] = None,
                               filter_next_7_days: bool = False) -> List[Game]:
        """
        ...
        Args:
            ...
            filter_next_7_days: If True, only return games within next 7 days from today
            
        Returns:
            List of unified games
        """
        # Apply 7-day filter if requested
        if filter_next_7_days:
            ...
            if not date_to:
                ...
                # Ensure date_to doesn't exceed 7 days from now
                max_date = now + timedelta(days=7)
                date_to = min(date_to, max_date)
...
async def example_usage():
    ...
    unified_client = create_unified_client(
        kalshi_config={
            'api_key': '266e5677-63fa-4fa5-8050-34522a468445',
            'private_key': '',
            'base_url': 'https://demo-api.kalshi.co/trade-api/v2'
        },
        odds_api_config={
            'api_key': '01d2f1b4913dc2f825d6133035bc61fe',
