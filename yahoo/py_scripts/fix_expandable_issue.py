#!/usr/bin/env python3
"""
Fix the expandable issue by making details generation fully dynamic.
"""

import json
import os

def create_fixed_standalone():
    """Create standalone HTML with properly working expandable details."""
    
    print("=== CREATING FIXED STANDALONE HTML ===\n")
    ...
    try:
        ...
        # Create JavaScript with dynamic details generation
        js_content = '''
// Fantasy Basketball Analysis App - Fixed Standalone Version
class FantasyAnalysisApp {
    ...
    async loadData() {
        try {
            ...
            if (window.EMBEDDED_DATA) {
                ...
                console.log('Sample player:', Object.values(this.players)[0].player_name);
            } else {
    ...
    renderPlayers() {
        ...
        // Create table with expandable rows (NO PRE-RENDERED DETAILS)
        let tableHtml = `
    ...
    generatePlayerDetails(playerId, player) {
        console.log('Generating details for:', player.player_name);
    ...
    togglePlayerRow(row) {
        ...
        if (!isExpanded) {
            ...
            const detailsContainer = document.getElementById(`details-${playerId}`);
            
            if (detailsContainer && player) {
                detailsContainer.innerHTML = this.generatePlayerDetails(playerId, player);
            }
...
<body>
    ...
    <script>
        ...
        # Write the file
        output_file = 'html_reports/standalone_final.html'
        ...
        print(f"Error creating fixed standalone HTML: {e}")
        return False

if __name__ == "__main__":
    create_fixed_standalone()